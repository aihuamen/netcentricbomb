...
const Box: React.FC<TheBox> = (props) => {
  ...
  const togglePic = () => {
    const yeey = props.isBomb ? process.env.PUBLIC_URL + '/yajuu.jpg' : process.env.PUBLIC_URL + '/pepe.jpg'
    setPic(yeey)
  }
  
  return (
    <img src={pic} onClick={togglePic} alt= 'Yee'/>
  );
}
...
const Board: React.FC = () => {
  ...
  return (
    <div>
      {bomb}
      <button onClick={() => setBomb(generateBoard)}>
        Reset
      </button>
    </div>
